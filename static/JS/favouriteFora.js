    ...
    $.ajax(
    {
        ...
        success: function (response)  
        {
            ...
            html +=     '<form method="post" class="box-svg" data-url-add="/accounts/add/favourite/' + response.slug_thread_league + 
                        ...
                        '</form>' +
                        '<label class="text-check-box" for="fav-thread-' + response.slug_thread_league + '">' +
                            '<div class="league-tchat">' +
                                '<div class="name-compe">' + response.thread_league + '</div>' +
                            '</div>' +
                            '<input type="checkbox" class="category-checkbox" id="fav-thread-' + response.slug_thread_league + '" data-url="/fora/display/threads/' + response.slug_thread_league + '">' +
                        '</label>' +
                    '</div>';
            ...
            $.getScript('/static/JS/threadsTchatsCategory.js', function() 
            {
            ...
            $.getScript('/static/JS/dropDownMenu.js', function() 
            {
                ...
                removeListenerCheckBox('.container-grid-cate');

                // Adding listener
                checkboxOnlyOne('.container-grid-cate', '.container-futur-threads', 'fora');
            });

            // Adding a listener on the star of the competition we have just set up
            $.getScript('/static/JS/favouriteFora.js', function() 
            {
